...
const valueOfAccTerminated = (array) => {
	for (i=0; i<array.length; i++) {
		let newInstruction = '';
		let tmpInstruction = array[i];

		switch (tmpInstruction.split(' ')[0]) {
        	case "acc" : continue
        	case "jmp" : newInstruction = "nop"; break
        	case "nop" : newInstruction = "jmp"; break
        }

        tmpInstruction = `${newInstruction} ${tmpInstruction.split(' ')[1]}`;
        array[i] = tmpInstruction;
        const { acc, isInfinite } = valueOfAcc(array);
