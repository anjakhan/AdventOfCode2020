...
targetSumTwo(arrayOfNums, 2020)

// ---------------------------------------------------------------------------------------------------

let targetSumThree = (array, sum) => {
	let nums = []

	for(let x in array) {
		for(let y in array) {
			for(let i in array) {
				if (array[x] + array[y] + array[i] === sum){
					let multiply = array[x] * array[y] * array[i]
					nums.push([array[x], array[y], array[i], multiply])
				}
			}
		}
	}

	return `The sum of ${nums[0][0]} + ${nums[0][1]} + ${nums[0][2]} is 2020. The numbers multiplied result into ${nums[0][3]}`
}

targetSumThree(arrayOfNums, 2020)